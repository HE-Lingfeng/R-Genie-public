...
class PhiDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # BKLM Module
        # visual_hidden_states = hidden_states[0:1, :, :].expand(hidden_states.size(0), -1, -1)
        # textual_hidden_states = hidden_states[1:, :, :].expand(hidden_states.size(0) - 1, -1, -1)
        # v2t_attn_outputs, v2t_attn_weights, present_key_value = self.self_attn(
        #     hidden_states=visual_hidden_states,
        #     attention_mask=attention_mask,
        #     position_ids=position_ids,
        #     past_key_value=past_key_value,
        #     output_attentions=output_attentions,
        #     use_cache=use_cache,
        # )
        # t2v_attn_outputs, t2v_attn_weights, present_key_value = self.self_attn(
        #     hidden_states=textual_hidden_states,
        #     attention_mask=attention_mask,
        #     position_ids=position_ids,
        #     past_key_value=past_key_value,
        #     output_attentions=output_attentions,
        #     use_cache=use_cache,
        # )
        # hidden_states = torch.cat((v2t_attn_outputs, t2v_attn_outputs), dim=0)
