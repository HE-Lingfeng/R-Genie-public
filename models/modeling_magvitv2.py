...
class VQGANEncoder(ModelMixin, ConfigMixin):
    ...
    def forward(self, x):
        ...
        #     self.conv_in.bias.data = self.conv_in.bias.data.to(torch.float32)
        hs = [self.conv_in(x)]
        for i_level in range(self.num_resolutions):
            for i_block in range(self.num_res_blocks[i_level]):
...
class MAGVITv2(ModelMixin, ConfigMixin):
    ...
    def get_code(self, pixel_values):
        # print(f"type(pixel_values) = {pixel_values.dtype}")
        # pixel_values = pixel_values.to(torch.bfloat16)
        hidden_states = self.encoder(pixel_values)
        # print(f"type(hidden_states) = {hidden_states.dtype}")
